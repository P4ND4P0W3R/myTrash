...
export const ModalSideTrash = (props: ModalProps) => {
	...
	const renderStatusButton = () => {
		if (props.selectedTrash.etat === 1) {
			return <p className="status bg-warning">Endommagé</p>;
		} else if (props.selectedTrash.full === 0) {
			return <p className="status greenSubBackground">Vide</p>;
		} else {
			return <p className="status redSubBackground">Pleine</p>;
		}
	};
	...
	const buttonStatus = () => {
		if (props.selectedTrash.etat === 1) {
			...
			return (
				<div>
					<div
						className="w-100 redSubBackground buttonStatus text-light"
						onClick={() => HandleChoice('Pleine')}
					>
						Pleine
					</div>
					<div
						className="w-100 mt-3 bg-warning buttonStatus"
						onClick={() => HandleChoice('Endommagé')}
					>
						Endommagée
					</div>
				</div>
			...
			return (
				<div>
					<div className="w-100 redSubBackground disabled buttonStatus text-light">
						Pleine
					</div>
					<div className="w-100 mt-3 bg-warning buttonStatus disabled">
						Endommagée
					</div>
				</div>
			);
	...
	return (
		<div className="sideBarContainer">
			<div className="col-xl-3 px-5 pt-5 h-100 containerModal">
				...
				<div className="location_info d-flex align-items-center pb-3">
					{' '}
					{/* Location div */}
			...
			<div
				className="col-xl-9 px-5 pt-5 h-100"
			...
			<ModalConfirmStatus
				...
				updateFullById={props.updateFullById}
				selectedTrash={props.selectedTrash}
			/>
		</div>
	);
};
